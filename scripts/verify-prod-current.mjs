/**
 * Verificación de producción — lee PRODUCT_VERSION y ventana UFC del repo.
 * Uso: npm run verify:prod
 */
...
const readUfcWindow = () => {
  try {
    const src = readFileSync("app/lib/ufc-week.ts", "utf8")
    const start = src.match(/windowStart:\s*"([^"]+)"/)?.[1]
    const end = src.match(/windowEnd:\s*"([^"]+)"/)?.[1]
    if (start && end) return { start, end }
  } catch {
    /* fallback editorial */
  }
  return { start: "2026-05-30", end: "2026-06-15" }
}

const expectedVersion = readExpectedVersion()
const ufcWindow = readUfcWindow()

console.log(`\nverify:prod — ${expectedVersion} @ ${BASE}\n`)
...
const inUfcWeek =
  madridDate >= ufcWindow.start && madridDate <= ufcWindow.end
